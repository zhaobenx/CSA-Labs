...
void printState(stateStruct state, int cycle)
{
    ...
    if (printstate.is_open())
    {
        ...
        if (1 == state.EX.nop)
        {
            printstate << "EX.Read_data1:\t"
                       << "X" << endl;
            printstate << "EX.Read_data2:\t"
                       << "X" << endl;
            printstate << "EX.Imm:\t"
                       << "X" << endl;
            printstate << "EX.Rs:\t"
            ...
            printstate << "EX.Rt:\t"
                       << "X" << endl;
            printstate << "EX.Wrt_reg_addr:\t"
                       << "X" << endl;
            printstate << "EX.is_I_type:\t"
                       << "X" << endl;
            printstate << "EX.rd_mem:\t"
                       << "X" << endl;
            printstate << "EX.wrt_mem:\t"
                       << "X" << endl;
            printstate << "EX.alu_op:\t"
                       << "X" << endl;
            printstate << "EX.wrt_enable:\t"
                       << "X" << endl;
        }
        else
        {
        ...
        if (1 == state.MEM.nop)
        {
            printstate << "MEM.ALUresult:\t"
                       << "X" << endl;
            printstate << "MEM.Store_data:\t"
                       << "X" << endl;
            printstate << "MEM.Rs:\t"
                       << "X" << endl;
            printstate << "MEM.Rt:\t"
                       << "X" << endl;
            printstate << "MEM.Wrt_reg_addr:\t"
                       << "X" << endl;
            printstate << "MEM.rd_mem:\t"
                       << "X" << endl;
            printstate << "MEM.wrt_mem:\t"
                       << "X" << endl;
            printstate << "MEM.wrt_enable:\t"
                       << "X" << endl;
        }
        else
        {
        ...
        if (1 == state.WB.nop)
        {
            printstate << "WB.Wrt_data:\t"
                       << "X" << endl;
            printstate << "WB.Rs:\t"
                       << "X" << endl;
            printstate << "WB.Rt:\t"
                       << "X" << endl;
            printstate << "WB.Wrt_reg_addr:\t"
                       << "X" << endl;
            printstate << "WB.wrt_enable:\t"
                       << "X" << endl;
        }
        else
        {
...
int main()
{
    ...
    while (1)
    {
        ...
        if (0 == state.EX.nop)
        {
            ...
            if (state.EX.INS == "beq") //Branch On Equal if(R[rs]==R[rt]) PC=PC+4+BrachAddr
            {
                ...
                if (state.EX.Read_data1 != state.EX.Read_data2)
                {
                    ...
                    newState.IF.PC = state.IF.PC.to_ulong() + bitset<30>(sign_extend(state.EX.Imm).to_string().substr(2, 30)).to_ulong() * 4 - 4;
                    newState.IF.nop = 0;
        ...
        if (0 == state.ID.nop)
        {
            ...
            else if (opcode == "000100") //Branch On not Equal if(R[rs]!=R[rt]) PC=PC+4+BrachAddr
            {
                newState.EX.INS = "beq";
